...
function renderStudentTable(data) {
    ...
    data.forEach(s => {
        tbody.innerHTML += `<tr><td><span class="badge bg-secondary">${s.rollNumber}</span></td><td class="small fw-bold text-dark">${s.name}</td><td class="small text-danger font-monospace">${s.pin}</td><td><button class="btn btn-xs btn-outline-primary btn-sm py-0 me-1" onclick="openEditPin('${s.rollNumber}', '${s.pin}')"><i class="fa-solid fa-pen"></i></button><button class="btn btn-xs btn-outline-danger btn-sm py-0" onclick="deleteStudent('${s.rollNumber}')"><i class="fa-solid fa-trash"></i></button></td></tr>`;
    });
}
...
window.saveNewPin = async function() {
    const rollNumber = document.getElementById('editPinRollNumber').value; const newPin = document.getElementById('newPinInput').value;
...
const addBtn = document.getElementById('addQuestionBtn'); if (addBtn) { addQuestionCard(); addBtn.addEventListener('click', addQuestionCard); }
...
window.viewDetails = function(subId) {
    ...
    if (sub.answers) { sub.answers.forEach((ans, index) => { const isCorr = ans.isCorrect; const badge = isCorr ? '<i class="fa-solid fa-circle-check text-success fs-5"></i>' : '<i class="fa-solid fa-circle-xmark text-danger fs-5"></i>'; const rowClass = isCorr ? '' : 'table-danger'; tbody.innerHTML += `<tr class="${rowClass}"><td class="fw-bold text-muted">${index + 1}</td><td class="small">${ans.numbersArray ? ans.numbersArray.join(', ') : 'N/A'}</td><td class="fw-bold fs-5 text-${isCorr ? 'success' : 'danger'}">${ans.studentAnswer}</td><td class="fw-bold fs-5 text-dark">${ans.correctAnswer}</td><td>${badge}</td></tr>`; }); }
