...
function addQuestionCard() {
    ...
    card.querySelector('.q-numbers').addEventListener('input', (e) => { card.querySelector('.q-answer').value = e.target.value.split(',').reduce((t, v) => t + (parseInt(v.trim()) || 0), 0); }); card.querySelector('.remove-btn').addEventListener('click', () => card.remove()); container.appendChild(card);
}
...
if (document.getElementById('createTestForm')) {
    document.getElementById('createTestForm').addEventListener('submit', async (e) => {
        e.preventDefault(); const title = document.getElementById('testTitle').value, timeLimit = document.getElementById('testTime').value;
        ...
        try { const res = await fetch(`${BASE_URL}/api/admin/tests`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ title, timeLimitMinutes: parseInt(timeLimit), questions: questionsArray, isActive: !isDraftMode, availableFrom, dueDate }) }); if (res.ok) { alert("Test Created!"); window.location.reload(); } } catch (e) { alert("Error"); }
    });
}
